...
import { CanvasItem, Scene, TextFileFormat, Model3DFormat } from './types'
...
import { generateUniqueName, getExistingImageNames, getExistingVideoNames, getExistingPdfNames, getExistingTextFileNames, getExistingModel3DNames } from './utils/imageNames'
...
function App() {
  ...
  const { videoPlaceholders, handleAddImage, handleAddVideo, handleAddPdf, handleAddTextFile, handleAddModel3D, handleUploadVideoAt } = useItemUpload({
  ...
  const { handleEmptyStateDrop } = useFileDrop({
    activeSceneId, isOffline, startOperation, endOperation,
    addImageAt, handleUploadVideoAt, addPdfAt, addTextFileAt, addModel3DAt,
  })
  ...
  const updateItem = useCallback(
    (id: string, changes: Partial<CanvasItem>, skipHistory?: boolean) => {
      ...
      if (!skipHistory) {
        ...
        const hasName = 'name' in changes && (item.type === 'image' || item.type === 'video' || item.type === 'pdf' || item.type === 'text-file' || item.type === 'model3d')
