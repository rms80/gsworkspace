...
- [x] create a new branch for this feature in github and switch to that branch
- [x] create separate files for the history system, in a subfolder /history
- [x] define an interface for history change records and the history system
- [x] Implement creating the history stack for the current scene in the main app
- [x] Implement the different types of change records and add them to the history in the right place
- [x] Implement serializing the history stack, and restoring it on load
- [x] Add undo/redo buttons to the Toolbar
- [x] Implement Ctrl+Z (undo) and Ctrl+Y/Ctrl+Shift+Z (redo) keyboard shortcuts
- [x] Truncate the History when a change is made while at some previous history state (IE normal undo history behaviour)
- [x] Create a Pull Request for this new feature branch
...
- [ ] Test creating HTML items programmatically (for now, no UI to create them)
  - Add a temporary test button or create via LLM response
